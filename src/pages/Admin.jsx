      ...
      >
        ...
        >
          <FormField label="Naam *">
            <input
              ...
              required
              style={inputCls}
              onFocus={e => { e.target.style.borderColor = '#6d3aed' }}
          ...
          <FormField label="Prijsinfo (bv. Gratis / Pro €20/m)">
            <input
              type="text"
              value={form.prijs_info}
              onChange={e => set('prijs_info', e.target.value)}
              style={inputCls}
              onFocus={e => { e.target.style.borderColor = '#6d3aed' }}
              onBlur={e => { e.target.style.borderColor = '#e2e4ea' }}
            />
          ...
          <FormField label="GDPR-status">
            <select
              ...
              onChange={e => set('gdpr_status', e.target.value)}
              style={inputCls}
            >
          ...
          <FormField label="GDPR-toelichting" full>
            <textarea
              value={form.gdpr_toelichting}
              onChange={e => set('gdpr_toelichting', e.target.value)}
              rows={2}
              style={{ ...inputCls, resize: 'vertical' }}
              onFocus={e => { e.target.style.borderColor = '#6d3aed' }}
              onBlur={e => { e.target.style.borderColor = '#e2e4ea' }}
          ...
          <FormField label="Use cases (komma-gescheiden)" full>
            <input
              ...
              onFocus={e => { e.target.style.borderColor = '#6d3aed' }}
              onBlur={e => { e.target.style.borderColor = '#e2e4ea' }}
              placeholder="Lesvoorbereiding, Samenvatten, Teksten schrijven"
            />
          </FormField>
          ...
          <div style={{ gridColumn: '1 / -1', display: 'flex', flexWrap: 'wrap', gap: 20 }}>
            {[
              ...
              <label
                key={key}
                style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 14, color: '#4b5166' }}
              >
                <input
                  type="checkbox"
                  checked={!!form[key]}
                  onChange={e => set(key, e.target.checked)}
                  style={{ width: 16, height: 16, accentColor: '#6d3aed' }}
                />
                {label}
              </label>
            ))}
        ...
        {message && (
          <p
            style={{
              marginTop: 16,
              fontSize: 14,
              color: message.startsWith('Fout') ? '#b91c1c' : '#15803d',
            }}
          >
            {message}
          </p>
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
          <button
            ...
            style={{
              padding: '10px 24px',
              backgroundColor: '#6d3aed',
              color: '#fff',
              border: 'none',
              borderRadius: 8,
              fontWeight: 600,
              fontSize: 14,
              cursor: saving || !form.naam ? 'not-allowed' : 'pointer',
              opacity: saving || !form.naam ? 0.6 : 1,
            }}
          >
...
export default function Admin() {
  ...
  async function fetchTools() {
    setLoading(true)
    const { data } = await supabaseAdmin.from('ai_tools').select('*').order('naam')
    setTools(data || [])
    setLoading(false)
  }
  ...
  async function saveTool(formData) {
    ...
    const { error } = id
      ? await supabaseAdmin.from('ai_tools').update(payload).eq('id', id)
      : await supabaseAdmin.from('ai_tools').insert(payload)
    setSaving(false)
    if (error) {
  ...
  async function toggleActief(tool) {
    await supabaseAdmin.from('ai_tools').update({ actief: !tool.actief }).eq('id', tool.id)
    await fetchTools()
  }
    ...
    return (
      ...
      >
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 24 }}>
            <div
              style={{
                ...
                height: 52,
                borderRadius: 14,
                background: 'linear-gradient(135deg, #6d3aed, #8b5cf6)',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                color: '#fff',
          ...
          <h1 style={{ fontSize: 20, fontWeight: 700, color: '#1e1f2e', textAlign: 'center', margin: 0 }}>
            Beheerdersomgeving
          </h1>
          <p style={{ fontSize: 13, color: '#8b90a7', textAlign: 'center', marginTop: 4, marginBottom: 24 }}>
            AI Tools — Miniemeninstituut Leuven
          </p>

          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#1e1f2e', marginBottom: 6 }}>
                Wachtwoord
              </label>
              <input
                type="password"
            ...
            {authError && (
              <p style={{ fontSize: 13, color: '#b91c1c', margin: 0 }}>{authError}</p>
            )}
            <button
              type="submit"
              style={{
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <a
              href="/"
              style={{ fontSize: 13, color: '#6d3aed', textDecoration: 'none' }}
            >
              ← Publieke weergave
            </a>
            <button
              onClick={handleLogout}
              style={{ fontSize: 13, color: '#8b90a7', background: 'none', border: 'none', cursor: 'pointer' }}
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                      <button
                        ...
                        onMouseEnter={e => { e.currentTarget.style.borderColor = '#6d3aed'; e.currentTarget.style.color = '#6d3aed' }}
                        onMouseLeave={e => { e.currentTarget.style.borderColor = '#e2e4ea'; e.currentTarget.style.color = '#4b5166' }}
                      >
                        Bewerken
                      </button>
                      <button
                        ...
                        style={{
                          padding: '7px 14px',
                          fontSize: 13,
                          border: `1px solid ${tool.actief ? '#e2e4ea' : '#bbf7d0'}`,
                          borderRadius: 8,
                          backgroundColor: '#fff',
                          color: tool.actief ? '#8b90a7' : '#15803d',
                          cursor: 'pointer',
                        }}
                      >
