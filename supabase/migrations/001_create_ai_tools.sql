...
-- Voer dit uit in de Supabase SQL Editor (eenmalig)
...
-- Iedereen mag lezen (anon)
DROP POLICY IF EXISTS "anon_read" ON ai_tools;
CREATE POLICY "anon_read" ON ai_tools
  FOR SELECT TO anon
  USING (true);

-- Seed 8 tools (alleen als tabel leeg is)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM ai_tools LIMIT 1) THEN
    ...
    VALUES
      ...
      (
        ...
        true,
        'Gratis (maandelijkse credits) / Creative Cloud',
        true, false,
        'OK',
      ...
      (
        ...
        'https://heygen.com',
        true,
        'Gratis (1 min/maand) / Pro $29/maand',
        true, false,
        'Opgelet',
